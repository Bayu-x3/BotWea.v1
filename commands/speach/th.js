...
module.exports = async function hanldeThCommands(message) {
 const usGet = message.body.substring(5);
 const encodedText = encodeURIComponent(usGet);

  if (!usGet) {
    message.reply("Cara Penggunaannya adalah dengan mengetik `.th (Kata-kata)`");
    return;
  }

  const apiUrl = `https://mysiraj.ddns.net/api/soundoftext?text=${encodedText}&lang=th-TH`;
