...
module.exports = async function handleJapanCommands(message) {
 const usGet = message.body.substring(5);
 const encodedText = encodeURIComponent(usGet);

  if (!usGet) {
    message.reply("Cara Penggunaannya adalah dengan mengetik `.jap (Kata-kata)`");
    return;
  }

  const apiUrl = `https://mysiraj.ddns.net/api/soundoftext?text=${encodedText}&lang=ja-JP`;
