...
module.exports = async function handleFrenchCommands(message) {
 const usGet = message.body.substring(5);
 const encodedText = encodeURIComponent(usGet);

  if (!usGet) {
    message.reply("Cara Penggunaannya adalah dengan mengetik `.fr (Kata-kata)`");
    return;
  }

  const apiUrl = `https://mysiraj.ddns.net/api/soundoftext?text=${encodedText}&lang=fr-FR`;
